...
你需要对三个（由助教处心积虑构造出的毫无意义且逻辑复杂的）函数进行[白盒测试](./whitebox.md)。你将对这些函数进行人工的控制流和数据流分析，画出函数的程序流程图，并给出尽可能的测试输入来达成对函数的语句覆盖、分支覆盖、条件组合覆盖。如果愿意，你还可以在[配套代码包](./code.md)里，使用[Boost.Test库](./boost.md)编写并实际运行你的测试，然后使用[llvm-cov工具](./llvm-cov.md)分析你的测试对待测函数的覆盖情况。如果你不会使用上述库/工具，我们还在文档中提供了简单的教学和进一步学习的指引。
...
选读内容，介绍了如何让G++/Clang++对你的C++程序进语句覆盖率插装，并使用llvm-cov对进行了覆盖率插装的程序进语句覆盖率检测。
